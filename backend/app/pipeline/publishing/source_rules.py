...
Adds the source-aware layer on top of the Slice 2 generic policy:
  ...
  * BleepingComputer is **not globally promoted**. It remains review-first by
    ...
    only** — never a stored credibility change. Generic vuln / patch /
    product-update / PoC / research items stay review-first.
  * All other sources have no special rule (effective credibility = stored).

Composition with the generic policy is provided by :func:`evaluate_v1_publish_decision`, 
which never upgrades a review/exclude into a publish. 
A qualifying (fraud-signal) BleepingComputer alert can auto-publish via 
its per-alert conditional credibility when the band and category gates also pass; 
a non-qualifying one is routed to review. The ``sources`` table is never read as a publish 
credential here and is never mutated — only the per-alert effective credibility is used.
"""
