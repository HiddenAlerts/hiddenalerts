 ...
 * Live list `risk` / `page` sync is handled by `nuqs` in `AlertsScreen`.
 */
...
/** Query string fragment for `/alerts` (no leading `?`). Omits defaults: all risks + page 1. */
export function buildAlertsListQueryString(risk: string, page: number): string {
...
/**
 * Validates `from` on alert detail URLs (encoded list query). Only `risk` and `page`
 * are honored; unknown keys are dropped.
 */
export function sanitizeAlertsReturnQueryString(encoded: string | null): string | null {
  ...
  try {
    ...
    const qs = buildAlertsListQueryString(risk, page);
    return qs === '' ? null : qs;
  } catch {
    return null;
  }
