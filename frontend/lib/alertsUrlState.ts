...
function normalizeAlertsSearchQueryFromSearchParams(
  raw: string | null,
): string {
  const v = (raw ?? '').trim();
  if (!v) return '';
  return v.length > MAX_SEARCH_QUERY_LEN ? v.slice(0, MAX_SEARCH_QUERY_LEN) : v;
}
...
export function buildAlertsListQueryString(
  ...
  const qNorm = normalizeAlertsSearchQueryFromSearchParams(
 ...
 * Only `risk`, `category`, `page`, and `q` are honored; unknown keys are dropped.
 */
export function sanitizeAlertsReturnQueryString(encoded: string | null): string | null {
  ...
  try {
    ...
    const q = normalizeAlertsSearchQueryFromSearchParams(probe.get('q'));
    const qs = buildAlertsListQueryString(risk, page, category, q || undefined);
    return qs === '' ? null : qs;
  } catch {
    return null;
  }
